 ...
 * VisitorReport.java
 *
 * Created on December 23, 2002, 5:13 PM
 */
 ...
 * @author Vinod Batra
 */
public final class VisitorReportAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			List<VisitorStatistics> users = userBean.getVisitorReport(startDate,
					endDate, uform.getOrderBy().equals("name") ? false : true);
			
			request.setAttribute("visitor_list", users);

		} catch (Exception e) {
			AceLogger.Instance().log(AceLogger.ERROR, AceLogger.SYSTEM_LOG,
					"VisitorReportAction.execute() : " + e.getMessage(), e);
		...
		return (mapping.findForward("show_visitor_report"));
	}
}
