...
public class LostPasswordPresenter {
	...
	public void finishedLostPassword(String message, MessageBoxPresenter.Severity sev) {
		MessageBoxPresenter.getInstance().show(
				ApplicationController.getMessages().LostPasswordPresenter_resetPassword(), message, sev,
	...
	private void showQuestionAnswerView(String name, HashMap<Integer, String> result) {
	...
	public void userNameSubmitted(final String name, String captcha, String captchaType) {
		...
		RequestBuilder builder = AceOperatorService.Util.getInstance().getSecurityQuestions(name.trim(), captcha.trim(),
				captchaType, new AsyncCallback<HashMap<Integer, String>>() {

					@Override
					public void onSuccess(HashMap<Integer, String> result) {
						if (result == null) {
							finishedLostPassword(
									ApplicationController.getMessages().LostPasswordPresenter_noUserOrEmail(),
									MessageBoxPresenter.Severity.SEVERE);
						} else if (result.isEmpty()) {
							finishedLostPassword(
									ApplicationController.getMessages().LostPasswordPresenter_noSecurityQuestions(),
									MessageBoxPresenter.Severity.SEVERE);
						} else {
	...
	public void answersSubmitted(String name, HashMap<Integer, String> answers, String captcha, String captchaType) {
		...
		RequestBuilder builder = AceOperatorService.Util.getInstance().resetPassword(name.trim(), captcha.trim(),
				captchaType, answers, ApplicationController.getInstance().getLocale(), new AsyncCallback<Void>() {

					@Override
					public void onSuccess(Void result) {
						finishedLostPassword(
								ApplicationController.getMessages().LostPasswordPresenter_successfulPasswordReset(),
	...
	private void handleError(Throwable caught) {
		...
		if ("databaseError".equals(error)) {
			errorResult(ApplicationController.getMessages().LostPasswordPresenter_databaseError());
			...
			finishedLostPassword(ApplicationController.getMessages().LostPasswordPresenter_resetPasswordEmailFailed(),
					MessageBoxPresenter.Severity.SEVERE);
		} else if ("unmatchedAnswers".equals(error)) {
			errorResult(ApplicationController.getMessages().LostPasswordPresenter_unmatchedAnswers());
		} else {
			errorResult(ApplicationController.getMessages().LostPasswordPresenter_resetFailed(error));
		}

	}

	public void errorResult(String message) {
		MessageBoxPresenter.getInstance().show(ApplicationController.getMessages().LostPasswordPresenter_warning(),
				message, MessageBoxPresenter.Severity.WARN, true);
	}

	public void validationError(String message) {
		MessageBoxPresenter.getInstance().show(ApplicationController.getMessages().LostPasswordPresenter_error(),
				message, MessageBoxPresenter.Severity.WARN, true);
	}
