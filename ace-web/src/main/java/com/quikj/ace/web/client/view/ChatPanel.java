...
public interface ChatPanel {
	...
	public static final String TYPING = "<span style='color:green'>"
			+ "<img src='" + Images.TYPING_TINY + "'></span>";
	...
	public static class Util {
		public static HTML formatChat(String from, long timeStamp,
				String message, String me, boolean smallSpace) {
			...
			String formattedDate = DateTimeFormat.getFormat(
					ClientProperties.getInstance().getStringValue(
							...
							ClientProperties.DEFAULT_TIME_FORMAT)).format(date);

			String spanStart = null;
			if (from == null) {
				spanStart = "<span class='ao-TranscriptMe'>";
				from = me;
			} else if (from.equals(ApplicationController.getMessages()
					.DesktopChatPanel_system())) {
			...
			HTML html;
			if (smallSpace) {
				html = new HTML(spanStart
						+ ApplicationController.getMessages()
								.DesktopChatPanel_chatMessageFromAtTimestamp(
										from, formattedDate) + " >></span> "
						+ message + "<br/>");
			} else {
				html = new HTML(spanStart
						+ ApplicationController.getMessages()
