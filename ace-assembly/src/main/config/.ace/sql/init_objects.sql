...
CREATE TABLE IF NOT EXISTS group_owner_tbl (
...
CREATE TABLE IF NOT EXISTS `blacklist_tbl` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS  cdr_logout_tbl (
...
CREATE TABLE IF NOT EXISTS  cdr_reg_login_tbl (
  loginid varchar(255) binary NOT NULL default '',
...
CREATE TABLE IF NOT EXISTS  cdr_session_disc_tbl (
...
CREATE TABLE IF NOT EXISTS  cdr_session_join_tbl (
...
CREATE TABLE IF NOT EXISTS  cdr_session_leave_tbl (
  session varchar(255) binary NOT NULL default '',
...
CREATE TABLE IF NOT EXISTS  cdr_session_setup_tbl (
  session varchar(255) binary NOT NULL default '',
...
CREATE TABLE IF NOT EXISTS  cdr_session_setup_resp_tbl (
  session varchar(255) binary NOT NULL default '',
  loginid varchar(255) binary NOT NULL default '',
...
CREATE TABLE IF NOT EXISTS  cdr_session_transfer_tbl (
  session varchar(255) binary NOT NULL default '',
...
CREATE TABLE IF NOT EXISTS  cdr_unreg_login_tbl (
  loginid varchar(255) binary NOT NULL default '',
...
CREATE TABLE IF NOT EXISTS  opm_operator_tbl (
  groupid varchar(64) binary NOT NULL default '',
  time_stamp datetime NOT NULL default 0,
...
CREATE TABLE IF NOT EXISTS canned_message_tbl
	(id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `feature_params_tbl` (
...
INSERT IGNORE INTO group_owner_tbl values ('operator','operator-group');
INSERT IGNORE INTO user_tbl (userid, password, domain, flags, fullname, address, addnl_info, unavail_xferto, gatekeeper, password_updated) 
	VALUES ('messagebox',password('a1b2c3d4'),'ace',0,'Message service feature','','Message service feature - handles offline messaging','', '', NOW());

INSERT IGNORE INTO canned_message_tbl (grp, description, message)
...
INSERT IGNORE INTO feature_tbl values (NULL, 'operator', 'ace', 'com.quikj.application.web.talk.feature.operator.Operator', 1);
INSERT IGNORE INTO feature_params_tbl values (LAST_INSERT_ID(), 'max-sessions', '1');
INSERT IGNORE INTO feature_params_tbl values (LAST_INSERT_ID(), 'max-operators', '10');
INSERT IGNORE INTO feature_params_tbl values (LAST_INSERT_ID(), 'max-queue-size', '20');
INSERT IGNORE INTO feature_params_tbl values (LAST_INSERT_ID(), 'display-wait-time', 'true');

INSERT IGNORE INTO feature_tbl values (NULL, 'messagebox', 'ace', 'com.quikj.application.web.talk.feature.messagebox.server.MessageBox', 1);
INSERT IGNORE INTO feature_params_tbl values (LAST_INSERT_ID(), 'from', 'messagebox@mydomain.com');
